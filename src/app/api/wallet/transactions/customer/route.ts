...
export async function GET(req: Request) {
  try {
    ...
    const backendUrl = `${process.env.BACKEND_URL}/api/transactions/my/payments`;
    ...
    return NextResponse.json({ transactions: response.data.data }, { status: 200 });
  } catch (error: any) {
    console.error("Wallet transactions API error:", error.response?.data || error);
